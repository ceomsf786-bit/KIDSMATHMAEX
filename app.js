...
async function loadTeacherAssignments(showLoading = true) {
  ...
  try {
    const { data, error } = await db
      ...
      .select("id,student_id,title,topic,mode,question_count,first_number_digits,second_number_digits,position,status,completed_at,created_at")
      .eq("group_id", CONFIG.groupId)
...
function renderTeacherAssignments() {
  ...
  rows.forEach((assignment, index) => {
    ...
    addCell(tr, assignment.topic === "operations" && assignment.first_number_digits && assignment.second_number_digits
      ? `${assignment.first_number_digits} digit${Number(assignment.first_number_digits) === 1 ? "" : "s"} / ${assignment.second_number_digits} digit${Number(assignment.second_number_digits) === 1 ? "" : "s"}`
      : "—");
